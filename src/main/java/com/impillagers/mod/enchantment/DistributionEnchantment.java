...
public class DistributionEnchantment extends Enchantment {
    ...
    public void onProjectileHit(PersistentProjectileEntity projectile, LivingEntity owner, Vec3d impactLocation) {
        ...
        int distributionLevel = EnchantmentHelper.getLevel(this, weapon);
    ...
    private float calculateDamage(ItemStack weapon) {
        int powerLevel = EnchantmentHelper.getLevel(net.minecraft.enchantment.Enchantments.POWER, weapon);
        return Math.round((float) (1.5 + (1.25 * (powerLevel + 1))) * 2) / 2.0f;
    }


    private List<StatusEffectInstance> getPotionEffects(ArrowEntity arrow) {
        List<StatusEffectInstance> effects = new ArrayList<>();

        // Get base potion effects
        Potion potion = ((ArrowEntityAccessor) arrow).getPotion();
        if (potion != Potions.EMPTY) {
            effects.addAll(potion.getEffects());
        }

        // Get custom potion effects from arrow's NBT
        effects.addAll(PotionUtil.getCustomPotionEffects(arrow.writeNbt(new NbtCompound())));

        return effects;
    }

    private void applyLingeringPotion(Entity projectile, List<StatusEffectInstance> effects, float radius, LivingEntity owner, World world, Vec3d impactLocation) {
    ...
    private void applyPotionEffectsToEntities(World world, Box area, List<StatusEffectInstance> effects) {
        for (LivingEntity entity : world.getEntitiesByClass(LivingEntity.class, area, e -> true)) {
            for (StatusEffectInstance effect : effects) {
    ...
    private void checkCombos(ItemStack weapon, float radius, Vec3d impactLocation, World world, Box area) {
        int flameLevel = EnchantmentHelper.getLevel(net.minecraft.enchantment.Enchantments.FLAME, weapon);
        int fireAspectLevel = EnchantmentHelper.getLevel(net.minecraft.enchantment.Enchantments.FIRE_ASPECT, weapon);
        int knockbackLevel = EnchantmentHelper.getLevel(net.minecraft.enchantment.Enchantments.KNOCKBACK, weapon);
        int punchLevel = EnchantmentHelper.getLevel(net.minecraft.enchantment.Enchantments.PUNCH, weapon);
        ...
        // Knockback combo
        int totalKnockback = knockbackLevel + punchLevel;
        if (totalKnockback > 0) {
            applyKnockbackEffects(totalKnockback, impactLocation, world, area);
        }
    }

    private void applyFireEffects(float radius, Vec3d impactLocation, World world, Box area) {
    ...
    private void applyKnockbackEffects(int totalKnockback, Vec3d impactLocation, World world, Box area) {
        for (LivingEntity entity : world.getEntitiesByClass(LivingEntity.class, area, e -> true)) {
        ...
        world.playSound(null, BlockPos.ofFloored(impactLocation), SoundEvents.ENTITY_PLAYER_ATTACK_KNOCKBACK, SoundCategory.BLOCKS, 2.0F, 1.0F);
    }
