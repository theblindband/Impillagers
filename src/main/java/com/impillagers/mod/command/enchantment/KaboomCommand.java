...
public class KaboomCommand implements ModCommandListener.IEffectHandler {
    ...
    public void handleEffect(ServerCommandSource source) {
        ...
        checkCombos(projectileEntity, enchantmentEntries, projectileEntity.getOwner());
    ...
    private void checkCombos(PersistentProjectileEntity projectileEntity, Set<Object2IntMap.Entry<RegistryEntry<Enchantment>>> enchantmentEntries, Entity owner) {
        ...
        LivingEntity intersectingEntity = projectileEntity.getWorld().getOtherEntities(projectileEntity, projectileEntity.getBoundingBox(), entity -> entity instanceof LivingEntity).stream().map(entity -> (LivingEntity) entity).findFirst().orElse(null);
        ...
        if (getEnchantmentLevel(enchantmentEntries, "Enchantment Flame") > 0) {
            BlockPos centerBlockPos = new BlockPos((int) Math.round(centerX), (int) Math.round(centerY), (int) Math.round(centerZ));
