    ...
    public static void registerModEvents() {
        ...
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            if (!player.getEntityWorld().isClient()) {
                ServerPlayNetworking.send(player, HudOverlayOpacityPayload.PACKET_ID, new HudOverlayOpacityPayload(0F).write(PacketByteBufs.create()));
            }
        });

        ServerPlayerEvents.AFTER_RESPAWN.register((oldPlayer, newPlayer, alive) -> {
            if (!newPlayer.getEntityWorld().isClient()) {
                ServerPlayNetworking.send(newPlayer, HudOverlayOpacityPayload.PACKET_ID, new HudOverlayOpacityPayload(0F).write(PacketByteBufs.create()));
            }
        });
    }
}
